...
#define CRID_UAS_ID_MAX_LEN    20     // UAS ID 鏈€澶ч暱搴?#define CRID_SSID_MAX_LEN      31     // SSID 鏈€澶ч暱搴?
...
typedef struct {
    ...
    float operator_lat;                      // 鎿嶄綔鍛樼含搴?    float operator_lon;                      // 鎿嶄綔鍛樼粡搴?    float operator_alt;                      // 鎿嶄綔鍛橀珮搴?(m)
    ...
    float time_counter;                      // 鏃堕棿璁℃暟鍣?} cn_crid_config_t;

/**
 * @brief 鍒濆鍖栦腑鍥?C-RID 閰嶇疆涓洪粯璁ゅ€? * @param config 鎸囧悜閰嶇疆缁撴瀯浣撶殑鎸囬拡
 */
